...
class InventoryDecisionSystem:
    def __init__(self, root):
        self.root = root
        self.root.title("智能库存决策系统 V4.7 - 归档索引版")
    ...
    def setup_ui(self):
        title_label = tk.Label(self.root, text="智能库存决策系统 V4.7", font=("Arial", 16, "bold"))
        title_label.pack(pady=5)
        
        subtitle = tk.Label(self.root, text="Auto-Scan | Archive by Channel & Timestamp | Keep All History", 
...
【Stock格式】支持新格式：Sku, PriceRadarVolume, IsDiscontinued, SouthIslandStock, NorthIslandStock（自动转换为长格式）
...
【操作提示】点击表头可排序 | 使用筛选框快速过滤 | 可折叠本说明节省空间
【颜色标识】🟪紫色(8-30天) | 🟩绿色(15天) | 🟦蓝色(30天) | ⬜白色(无销售) | 🔴红色(下单备货) | 🟡黄色(催促发货)"""
        ...
        # 分析结果表格
        result_frame = tk.LabelFrame(self.root, text="分析结果（点击表头可排序）", padx=10, pady=10)
        result_frame.pack(fill="both", expand=True, padx=20, pady=5)
        
        columns = ("SKU", "地区", "在库库存", "在途库存", "总量库存", 
        ...
        col_widths = [80, 60, 70, 70, 70, 65, 65, 65, 75, 85, 80, 80, 90, 80, 75, 80, 180]
    ...
    def on_header_click(self, col):
        col_map = {
            "SKU": "Sku", "地区": "Region", "在库库存": "在库库存", "在途库存": "在途库存",
        ...
        if not df_col or self.result_data is None:
            return
        if self.sort_column == df_col:
            self.sort_ascending = not self.sort_ascending
        else:
            self.sort_column = df_col
            self.sort_ascending = True
        self.refresh_display()
    ...
    def refresh_display(self):
        ...
        for _, row in df.iterrows():
            values = (
                row['Sku'], row['Region'], f"{row['在库库存']:.0f}", f"{row['在途库存']:.0f}",
    ...
    def run_analysis_core(self, silent=False, auto_export=False, channel_override=None):
        try:
            ...
            df = pd.merge(all_skus, stock_df[['Sku', 'Region', '在库库存', 'PriceRadarVolume', 'IsDiscontinued']], 

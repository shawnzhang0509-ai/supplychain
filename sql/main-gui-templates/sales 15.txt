...
CheckinWindows AS (
    SELECT
        ...
        CASE
            WHEN CAST(GETDATE() AS DATE) BETWEEN DATEADD(day, 1, re.RestockDate) AND DATEADD(day, 14, re.RestockDate)
                THEN CAST(GETDATE() AS DATE)
            ELSE DATEADD(day, 15, re.RestockDate)
        END AS SampleEnd,
        CASE
            WHEN re.rn = 1
                 AND CAST(GETDATE() AS DATE) BETWEEN DATEADD(day, 1, re.RestockDate) AND DATEADD(day, 14, re.RestockDate)
                THEN 1 ELSE 0
        END AS IsPartialPeriod
    FROM RestockEvents_Ranked re
    WHERE re.rn BETWEEN 1 AND 3
),
